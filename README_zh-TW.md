...
### 入口網站語言切換
- **使用者下拉選單中的地球圖示** — 當安裝多種語言時，入口網站使用者下拉選單中會顯示語言切換區塊
- **一鍵切換** — 點擊任何已安裝的語言即可立即切換入口網站介面
- **使用者偏好持久化** — 語言選擇會儲存至使用者設定，後續造訪時自動套用
- **智慧重導向** — 切換後自動重導向回相同頁面，並套用新語言
- **獨立於網站設定** — 使用 `_frontend_pre_dispatch` 強制套用使用者語言偏好，無需修改網站設定

### 可編輯專案入口
- **統一任務列表** — 擁有完整編輯權限的專案協作者與唯讀使用者看到相同的入口風格卡片佈局，取代原本的 iframe OWL 看板
- **新增任務 / 新增階段** — 透過彈出視窗表單直接從入口建立任務和階段
- **欄位選擇器** — 齒輪圖示下拉選單可切換任務卡片顯示的欄位（12 個欄位，儲存於 localStorage）
- **條件式欄位** — 來自 `hr_timesheet` 和 `sale_timesheet` 的欄位在未安裝時自動隱藏
- **編輯任務彈出視窗** — 點擊任何鉛筆圖示開啟包含名稱、階段、截止日期和描述欄位的彈出視窗
- **子任務管理** — 在任務詳細頁面直接查看、新增（含名稱、階段、截止日期、描述）和刪除子任務
- **常態收合搜尋列** — 可編輯專案頁面的排序 / 分組 / 搜尋控制項收合於篩選按鈕後方
- **子任務麵包屑** — 檢視子任務時顯示父任務的正確麵包屑導航
- **無額外依賴** — 使用 `try/except` 匯入和 `forcecreate=False` 模式確保跨模組相容性

### 入口頁面風格化
- **WoowTech 設計** — 以 WoowTech 品牌風格重新設計所有入口頁面（#71639e）
...
### 語言切換器
使用者下拉選單中的地球圖示，可在已安裝的語言間快速切換。

<p align="center">
  <img src="docs/screenshots/language-switcher.png" alt="語言切換器" width="720"/>
</p>
...
| `POST` | `/my/activity/done` | 完成待辦活動 |
| `GET` | `/my/set_lang/<lang_code>` | 切換入口網站語言並重導向 |
| `POST` | `/my/projects/<id>/add_task` | 在專案中建立任務 |
| `POST` | `/my/projects/<id>/add_stage` | 為專案建立階段 |
| `POST` | `/my/projects/<id>/task/<id>/update` | 更新任務欄位（名稱、階段、截止日期、描述） |
| `POST` | `/my/projects/<id>/task/<id>/add_subtask` | 建立連結至父任務的子任務 |
| `POST` | `/my/projects/<id>/task/<id>/delete_subtask` | 刪除子任務 |
...
### v18.0.2.2.0 (2026-05)
- 可編輯專案入口：專案協作者的任務列表頁和任務詳細頁
- 新增任務 / 新增階段彈出視窗表單
- 欄位選擇器（齒輪圖示）可切換 12 個卡片欄位（localStorage 持久化）
- 編輯任務彈出視窗：名稱、階段、截止日期、描述
- 子任務管理：新增（含名稱、階段、截止日期、描述）、查看和刪除
- 可編輯專案頁面常態收合搜尋列
- 子任務麵包屑導航含父任務連結
- 條件式 hr_timesheet / sale_timesheet 欄位（未安裝時自動隱藏）
- 所有新入口字串的繁體中文翻譯
- 修正 .pot 檔案以包含所有可翻譯項目供 Odoo 18 翻譯匯入

### v18.0.2.1.0 (2026-05)
- 入口網站使用者下拉選單新增語言切換器（地球圖示）
- 新增 `/my/set_lang/<lang_code>` 路由用於語言切換
- 透過 `_frontend_pre_dispatch` 覆寫尊重使用者語言偏好
- 智慧重導向：切換語言後自動移除舊語言前綴
- 更新所有截圖為新紫色主題
